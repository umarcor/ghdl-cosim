...
As mentioned in the introduction, in VASG's TWIKI there is a proposal by Peter Flake since 2011 titled [DpiProposal](http://www.eda-twiki.org/cgi-bin/view.cgi/P1076/DpiProposal). The main purpose of that proposal was to allow execution of VHDL subprograms from foreign languages. However, since it is an enhancement to VHPI, other related features are being added to that proposal. In fact, during the last months, this proposal and the one by Peter Flake are being blended, as a result of the discussions in the VASG. Hence, this subsection contains the relevant differences between both proposals, which have not been polished yet.

> Accessed on 2020/09/02
>
...
IMHO, attribute `EXPORT` is redundant. `attribute FOREIGN of G: function is "EXPORT_C g";` is already expressive, and an existing attribute is reused.
...
I feel that this falls short in providing a standard interface. Vendors providing potentially incompatible C headers, would force users to write several versions of the same C functions, just to comply with vendor details. Instead, I believe that vendors should make that "bridging" C header part of their codebase. So, a single VHDL source and a single matching C source should work equally with any simulator/tool. Moreover, this is coherent with how VHPI was conceived. Having 10+ vector types defined in VHPI but not in FFI/DPI feels incoherent. As explained above, defining the structure to derive complex/composite types seems feasible.
