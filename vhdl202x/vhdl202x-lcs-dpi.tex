...
\date{Last updated on \href{https://github.com/umarcor/ghdl-cosim/commit/}{} and generated on 2026/03/26}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{typedef} \KeywordTok{struct}\NormalTok{ vhpiForeignDataS }\OperatorTok{\{}
\NormalTok{  vhpiForeignKindT kind}\OperatorTok{;}
  \DataTypeTok{char} \OperatorTok{*}\NormalTok{ libraryName}\OperatorTok{;}
  \DataTypeTok{char} \OperatorTok{*}\NormalTok{ modelName}\OperatorTok{;}
  \DataTypeTok{void} \OperatorTok{(*}\NormalTok{elabf}\OperatorTok{)(}\DataTypeTok{const} \KeywordTok{struct}\NormalTok{ vhpiCbDataS }\OperatorTok{*}\NormalTok{cb\_data\_p}\OperatorTok{);}
  \DataTypeTok{void} \OperatorTok{(*}\NormalTok{execf}\OperatorTok{)(}\DataTypeTok{const} \KeywordTok{struct}\NormalTok{ vhpiCbDataS }\OperatorTok{*}\NormalTok{cb\_data\_p}\OperatorTok{);}
\OperatorTok{\}}\NormalTok{ vhpiForeignDataT}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{typedef} \KeywordTok{struct}\NormalTok{ vhpiCbDataS }\OperatorTok{\{}
  \DataTypeTok{int32\_t}\NormalTok{ reason}\OperatorTok{;}                              \CommentTok{/* callback reason */}
  \DataTypeTok{void} \OperatorTok{(*}\NormalTok{cb\_rtn}\OperatorTok{)} \OperatorTok{(}\DataTypeTok{const} \KeywordTok{struct}\NormalTok{ vhpiCbDataS }\OperatorTok{*);} \CommentTok{/* call routine */}
\NormalTok{  vhpiHandleT obj}\OperatorTok{;}                             \CommentTok{/* trigger object */}
\NormalTok{  vhpiTimeT }\OperatorTok{*}\NormalTok{time}\OperatorTok{;}                             \CommentTok{/* callback time */}
\NormalTok{  vhpiValueT }\OperatorTok{*}\NormalTok{value}\OperatorTok{;}                           \CommentTok{/* trigger object value */}
  \DataTypeTok{void} \OperatorTok{*}\NormalTok{user\_data}\OperatorTok{;}                             \CommentTok{/* pointer to user data to be passed to the callback function */}
\OperatorTok{\}}\NormalTok{ vhpiCbDataT}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{procedure} \DecValTok{vplot}\NormalTok{ (x}\OtherTok{:} \DataTypeTok{std\_logic\_vector}\NormalTok{(}\DecValTok{0} \OtherTok{to} \DecValTok{15}\NormalTok{)) }\ControlFlowTok{is}
\KeywordTok{begin} \KeywordTok{report} \StringTok{"VFFI vplot"} \KeywordTok{severity} \KeywordTok{failure}\NormalTok{; }\KeywordTok{end;}
\OtherTok{attribute}\NormalTok{ foreign }\KeywordTok{of}\NormalTok{ vplot }\OtherTok{:} \KeywordTok{procedure} \KeywordTok{is} \StringTok{"VFFI plot"}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{void}\NormalTok{ plot}\OperatorTok{(}\DataTypeTok{int}\OperatorTok{*}\NormalTok{ x}\OperatorTok{)} \OperatorTok{\{}
...
\begin{Shaded}
\begin{Highlighting}[]
\DataTypeTok{void} \OperatorTok{(*}\NormalTok{plot}\OperatorTok{)(}\DataTypeTok{int}\OperatorTok{*);}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\OtherTok{attribute}\NormalTok{ foreign }\KeywordTok{of}\NormalTok{ plot}\OtherTok{:} \KeywordTok{procedure} \KeywordTok{is} \StringTok{"VFFI \_protoype\_"}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
...
VHDL subprograms might be overloaded, while C subprograms cannot. \st{It should be possible to have a couple of VHDL subprograms with the same name, each mapped to a different foreign subprogram}. Tristan pointed out that it is possible to provide a signature when defining an attribute. See VHDL 2019 LRM, 7.2 Attribute specification. Hence, although verbose, the following works:

\begin{Shaded}
\begin{Highlighting}[]
  \AttributeTok{function} \DecValTok{xyce\_run}\NormalTok{(}
\NormalTok{    id      }\OtherTok{:} \DataTypeTok{string}\NormalTok{;}
\NormalTok{    reqTime }\OtherTok{:} \DataTypeTok{real}
\NormalTok{  ) }\ControlFlowTok{return} \DataTypeTok{integer}\NormalTok{;}
  \OtherTok{attribute}\NormalTok{ foreign }\KeywordTok{of}\NormalTok{ xyce\_run }\OtherTok{:} \KeywordTok{function} \KeywordTok{is} \StringTok{"VHPIDIRECT xhdl\_run"}\NormalTok{;}

  \AttributeTok{function} \DecValTok{xyce\_run}\NormalTok{(}
\NormalTok{    id      }\OtherTok{:} \DataTypeTok{string}\NormalTok{;}
\NormalTok{    reqTime }\OtherTok{:} \DataTypeTok{real}\NormalTok{;}
\NormalTok{    arrTime }\OtherTok{:} \DataTypeTok{real\_vector}\NormalTok{;}
\NormalTok{    arrVolt }\OtherTok{:} \DataTypeTok{real\_vector}
\NormalTok{  ) }\ControlFlowTok{return} \DataTypeTok{integer}\NormalTok{;}
  \OtherTok{attribute}\NormalTok{ foreign }\KeywordTok{of}\NormalTok{ xyce\_run}\OtherTok{[}
    \DataTypeTok{string}\OtherTok{,}
    \DataTypeTok{real}\OtherTok{,}
    \DataTypeTok{real\_vector}\OtherTok{,}
    \DataTypeTok{real\_vector}
\NormalTok{    return }\DataTypeTok{integer}
  \OtherTok{]} \OtherTok{:} \KeywordTok{function} \KeywordTok{is} \StringTok{"VHPIDIRECT xhdl\_run\_1D"}\NormalTok{;}

  \OtherTok{type}\NormalTok{ arr2D\_t }\OtherTok{is} \KeywordTok{array}\NormalTok{ (}\DataTypeTok{natural} \KeywordTok{range} \OtherTok{\textless{}\textgreater{},} \DataTypeTok{natural} \KeywordTok{range} \OtherTok{\textless{}\textgreater{}}\NormalTok{) }\KeywordTok{of} \DataTypeTok{real}\NormalTok{;}
  \AttributeTok{function} \DecValTok{xyce\_run}\NormalTok{(}
\NormalTok{    id      }\OtherTok{:} \DataTypeTok{string}\NormalTok{;}
\NormalTok{    reqTime }\OtherTok{:} \DataTypeTok{real}\NormalTok{;}
\NormalTok{    arr2D   }\OtherTok{:}\NormalTok{ arr2D\_t}
\NormalTok{  ) }\ControlFlowTok{return} \DataTypeTok{integer}\NormalTok{;}
  \OtherTok{attribute}\NormalTok{ foreign }\KeywordTok{of}\NormalTok{ xyce\_run}\OtherTok{[}
    \DataTypeTok{string}\OtherTok{,}
    \DataTypeTok{real}\OtherTok{,}
\NormalTok{    arr2D\_t}
\NormalTok{    return }\DataTypeTok{integer}
  \OtherTok{]} \OtherTok{:} \KeywordTok{function} \KeywordTok{is} \StringTok{"VHPIDIRECT xhdl\_run\_2D"}\NormalTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{typedef} \KeywordTok{struct}\NormalTok{ vhpiValueS }\OperatorTok{\{}
\NormalTok{  vhpiFormatT format}\OperatorTok{;}
  \DataTypeTok{size\_t}\NormalTok{ bufSize}\OperatorTok{;}
  \DataTypeTok{int32\_t}\NormalTok{ numElems}\OperatorTok{;}
\NormalTok{  vhpiPhysT unit}\OperatorTok{;}
  \KeywordTok{union} \OperatorTok{\{}
\NormalTok{    vhpiEnumT      enumv}\OperatorTok{,}      \OperatorTok{*}\NormalTok{enumvs}\OperatorTok{;}
\NormalTok{    vhpiSmallEnumT smallenumv}\OperatorTok{,} \OperatorTok{*}\NormalTok{smallenumvs}\OperatorTok{;}
\NormalTok{    vhpiIntT       intg}\OperatorTok{,}       \OperatorTok{*}\NormalTok{intgs}\OperatorTok{;}
\NormalTok{    vhpiLongIntT   longintg}\OperatorTok{,}   \OperatorTok{*}\NormalTok{longintgs}\OperatorTok{;}
\NormalTok{    vhpiRealT      real}\OperatorTok{,}       \OperatorTok{*}\NormalTok{reals}\OperatorTok{;}
\NormalTok{    vhpiSmallPhysT smallphys}\OperatorTok{,}  \OperatorTok{*}\NormalTok{smallphyss}\OperatorTok{;}
\NormalTok{    vhpiPhysT      phys}\OperatorTok{,}       \OperatorTok{*}\NormalTok{physs}\OperatorTok{;}
\NormalTok{    vhpiTimeT      time}\OperatorTok{,}       \OperatorTok{*}\NormalTok{times}\OperatorTok{;}
\NormalTok{    vhpiCharT      ch}\OperatorTok{,}         \OperatorTok{*}\NormalTok{str}\OperatorTok{;}
    \DataTypeTok{void}           \OperatorTok{*}\NormalTok{ptr}\OperatorTok{,}       \OperatorTok{**}\NormalTok{ptrs}\OperatorTok{;}
  \OperatorTok{\}}\NormalTok{ value}\OperatorTok{;}
\OperatorTok{\}}\NormalTok{ vhpiValueT}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{typedef} \KeywordTok{struct} \OperatorTok{\{}
  ...
  \DataTypeTok{int}\NormalTok{ dim}\OperatorTok{;}
  \DataTypeTok{void}\OperatorTok{*}\NormalTok{ bounds}\OperatorTok{;}
\OperatorTok{\}}\NormalTok{ fat\_pointer\_t}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{typedef} \KeywordTok{struct} \OperatorTok{\{}
  \DataTypeTok{int32\_t}\NormalTok{ left}\OperatorTok{;}
  \DataTypeTok{int32\_t}\NormalTok{ right}\OperatorTok{;}
  \DataTypeTok{int32\_t}\NormalTok{ dir}\OperatorTok{;}
\OperatorTok{\}}\NormalTok{ range\_t}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\KeywordTok{typedef} \KeywordTok{struct} \OperatorTok{\{}
  ...
  \DataTypeTok{int}\NormalTok{ dim}\OperatorTok{;}
\NormalTok{  range\_t}\OperatorTok{*}\NormalTok{ bounds}\OperatorTok{;}
\OperatorTok{\}}\NormalTok{ fat\_pointer\_natural\_multidim\_array\_of\_stdv\_t}\OperatorTok{;}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{\{}
\AttributeTok{  }\FunctionTok{data}\KeywordTok{:}\AttributeTok{ char*}\KeywordTok{,}
\AttributeTok{  }\FunctionTok{dim}\KeywordTok{:}\AttributeTok{ }\DecValTok{3}\KeywordTok{,}
\AttributeTok{  [}
\AttributeTok{    \{}
\AttributeTok{        }\FunctionTok{left}\KeywordTok{:}\AttributeTok{ int}\KeywordTok{,}
\AttributeTok{        }\FunctionTok{right}\KeywordTok{:}\AttributeTok{ int}\KeywordTok{,}
\AttributeTok{        }\FunctionTok{dir}\KeywordTok{:}\AttributeTok{ int}\KeywordTok{,}
...
\tightlist
\item
  \st{Attribute FOREIGN should allow to define more than one foreign ``language'' or ``implementation''. For example, it should be possible to use the same VHDL source to co-simulate with either GHDL's VHPIDIRECT or Mentor Graphics' FLI. Currently, the format of the attribute string is not standarized, and the implementation of those tools is incompatible. Apart from that, VHDL sources are fully compatible}.
\item
...
\item
  \st{Is it possible to support varargs so that calling \mbox{\texttt{printf}} from VHDL is a ``built-in'' feature in the language?}
\end{itemize}
