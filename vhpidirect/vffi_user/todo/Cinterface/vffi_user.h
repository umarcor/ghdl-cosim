...
typedef struct ghdl_NaturalDimArr_t {
  void* base;
  bounds_t* bounds;
} ghdl_NaturalDimArr_t;

void printfat(ghdl_NaturalDimArr_t* ptr) {
    printf("fat.base: %p\n", ptr->base);
    printf("fat.bounds: %p\n", ptr->bounds);
...
char* fatptrToString(ghdl_NaturalDimArr_t* ptr) {
  assert(ptr != NULL);
...
void fatptrToArray(ghdl_NaturalDimArr_t* ptr, void** vec, int* len, int num) {
  assert(ptr != NULL);
  assert(ptr->bounds != NULL);
  *vec = ptr->base;
...
ghdl_NaturalDimArr_t* fatptrFromString(char* str) {
  uint32_t len = strlen(str);
  ghdl_NaturalDimArr_t* fat = malloc(sizeof(ghdl_NaturalDimArr_t));
...
ghdl_NaturalDimArr_t* fatptrFromIntegerArray(void* vec, int* len, int num) {
  ghdl_NaturalDimArr_t *fat = malloc(sizeof(ghdl_NaturalDimArr_t));
  fat->base = vec;
  fat->bounds = malloc(sizeof(bounds_t));
  bounds_t* b = fat->bounds;
